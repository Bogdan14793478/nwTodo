...
async function add(req, res) {
  ...
  try {
    //   // const { task, index, parentId } = req.body;
    //   const todo = new Todo({
    //     task: req.body.task,
...
async function remove(req, res) {
  try {
    const { index, task, _id, parentId } = req.body;
...
//         (task) => task.index === taskMoveIndex - 1 && task.parentId === parentId
//       );
...
async function removeSublist(_id, Todo) {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (!!task) await removeSublist(task._id, Todo);
    else return;
  }
}
